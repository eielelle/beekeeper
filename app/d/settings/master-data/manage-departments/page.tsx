...
export default function DepartmentsPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string | number) => deleteDepartment(id.toString()),
    onSuccess: () => {
  ...
  return (
    <DataTable
      title="Departments"
      description="Manage company departments and organization units"
      ...
      renderForm={({ id, onClose }) => (
        <div className="max-h-[80vh] overflow-y-auto pr-1">
          <DepartmentForm editId={id?.toString()} onClose={onClose} />
        </div>
      )}
      onDelete={async (id) => {
      ...
      getItemDisplayName={(item) => `${item.code} (${item.name})`}
    />
  )
}
